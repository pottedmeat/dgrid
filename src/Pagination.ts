...
class Pagination extends PaginationBase<PaginationProperties> {
	render (): DNode {
		...
		if (pages > 1) {
			...
			pageLinks.push(w(PageLink, { key: '1', disabled: isFirstPage, page: 1, onPageRequest }));

			if (page > 4) {
				pageLinks.push(v('span', { key: 'skip1', classes: this.classes(css.pageSkip) }, [ '...' ]));
			}
			if (page > 3) {
				pageLinks.push(w(PageLink, { key: String(page - 2), page: page - 2, onPageRequest }));
			}
			if (page > 2) {
				pageLinks.push(w(PageLink, { key: String(page - 1), page: page - 1, onPageRequest }));
			}
			if (!isFirstPage && !isLastPage) {
				pageLinks.push(w(PageLink, { key: String(page), disabled: true, page: page, onPageRequest }));
			}
			if (page + 1 < pages) {
				pageLinks.push(w(PageLink, { key: String(page + 1), page: page + 1, onPageRequest }));
			}
